...
void solve() {
  ...
  rdvin(b, N);
  for (int i = 0; i < N; i++)
    tie(b[i], a[i]) = minmax({a[i], b[i]});

  dbg(a);
  dbg(b);
  int mss = 0, ss = 0, msi = 0, msj = 0, csi = 0, csj = 0;
  for (int i = 0; i < N; i++) {
    if (ss + a[i] > 0)
      ss = ss + a[i], csj = i;
    else
      csi = i, csj = i, ss = 0;
    if (ss > mss)
      mss = ss, msi = csi, msj = csj;
  }

  int mn = 0;
  rep(i, msi, msj + 1) { mn = min(mn, a[i]); }
  ...
  for (int i = 0; i < N; i++) {
    if (ss + a[i] > 0)
      ...
      csi = i, csj = i, ss = 0;
    if (ss > nmss)
      nmss = ss, msi = csi, msj = csj;
  }

  cout << max(mss - mn, mss + nmss) << nl;
}
