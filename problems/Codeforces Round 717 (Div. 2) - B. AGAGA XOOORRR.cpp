...
void solve() {
  ...
  auto ans = [&](auto &ans, int l, int h, int k, int xr) -> bool {
    if (l > h)
      return false;
    if (l == h)
      return k == a[l];
    if (l == h + 1)
      return (k == a[l] && k == a[h]) || k == xr;
    int lxr = 0;
    for (int i = l + 1; i <= h; i++) {
      xr ^= a[i - 1];
      lxr ^= a[i - 1];
      if (xr == k && xr == lxr)
        return true;
      if (a[i] == k && ans(ans, l, i - 1, a[i], lxr) &&
          ans(ans, i + 1, h, a[i], xr ^ a[i])) {
        return true;
      }
    }
    return false;
  ...
  for (int i = l + 1; i <= h; i++) {
    ...
    if (ans(ans, l, i - 1, a[i], lxr) && ans(ans, i + 1, h, a[i], xr ^ a[i])) {
      cout << "YES" << nl;
      return;
    }
  }
