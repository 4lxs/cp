...
void solve() {
    ...
    vi d(N+1), p(N + 1);
    auto dfs = [&](auto& dfs, int u, int pr, int de) -> void {
        d[u] = de;
        p[u] = pr;
        for(auto o : G[u]) if (o != pr) dfs(dfs, o, u, de + 1);
    };
    dfs(dfs, 1, 1, 0);
    vvi tbl = treeJump(p);
    ...
    rep(q, 0, Q) {
        rdi(A, B, E);
        int anc = lca(tbl, d, A, B);
        dbg(A, B, anc, d[anc]);
        if (d[A] - d[anc] >= E) {
          cout << jmp(tbl, A, E) << nl;
        } else if (E < d[A] + d[B] - 2 * d[anc]) {
          int eleft = E - (d[A] - d[anc]);
          cout << jmp(tbl, B, d[B] - d[anc] - eleft) << nl;
        } else {
          cout << B << nl;
        }
    }
